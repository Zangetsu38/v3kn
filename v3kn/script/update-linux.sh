...
# Server now stops itself before running this script.
# Keep only a light safety check for a stale screen session.
if screen -list 2>/dev/null | grep -qE "[0-9]+\\.${SCREEN_NAME}[[:space:]]"; then
    echo "Stale screen session detected, stopping it..."
