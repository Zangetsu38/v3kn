...
static void fill_presence_fields(json &status_obj, const std::string &npid, bool include_last_activity, const std::string &language) {
    std::lock_guard<std::mutex> lock(online_users_mutex);
    ...
    if (is_online) {
        ...
        if (now_playing_it != online_now_playing.end())
            now_playing = get_title_name(now_playing_it->second, language);
    }
...
void handle_friend_add(const httplib::Request &req, httplib::Response &res) {
    ...
    if (has_received_request || has_target_sent_request) {
        ...
        create_friendship_established_activity(npid, target_npid);
...
void handle_friend_list(const httplib::Request &req, httplib::Response &res) {
    ...
    if (group.empty()) {
        log("Missing group parameter on friends list request for NPID " + npid);
    ...
    if (language.empty()) {
        log("Missing sys_lang parameter on friends list request for NPID " + npid);
    ...
    if (group == "friends") {
        ...
        for (const auto &f : friends) {
            ...
            json entry = f;
            const std::string friend_npid = f["npid"].get<std::string>();
            fill_presence_fields(entry, friend_npid, false, language);
        ...
        self_entry["since"] = 0;
        fill_presence_fields(self_entry, npid, false, language);
...
void handle_friend_profile(const httplib::Request &req, httplib::Response &res) {
    ...
    const std::string language = req.get_param_value("sys_lang");
    if (language.empty()) {
        log("Missing sys_lang parameter on friend profile request for NPID " + npid);
        res.set_content("ERR:MissingLanguage", "text/plain");
        return;
    }
    ...
    if (has_friend(blocked, target_npid)) {
        ...
        fill_presence_fields(response, target_npid, false, language);
    ...
    // Include last_updated from profile if available (for friends, this is used to trigger updates on the client when profile changes)
    const fs::path profile_path{ fs::path("v3kn") / "Users" / target_npid / "profile.json" };
    if (fs::exists(profile_path)) {
        ...
        try {
            ...
            log("Corrupted profile for NPID " + target_npid + " requested by NPID " + npid);
        }
    } else
        log("Profile not found for NPID " + target_npid + " requested by NPID " + npid);
...
void handle_friend_presence(const httplib::Request &req, httplib::Response &res) {
    ...
    if (status_changed) {
        ...
        const std::string now_playing_name = get_title_name(now_playing, "");
        log("Now playing updated for: " + npid + " -> " + now_playing + " (" + now_playing_name + ")");
    }
