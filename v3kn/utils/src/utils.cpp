...
static json titles_cache = json{ { "titles", json::object() } };
...
json load_titles() {
    std::ifstream f("v3kn/titles.json");
    if (!f.is_open())
        return json{ { "titles", json::object() } };

    json db;
    f >> db;
    if (!db.contains("titles") || !db["titles"].is_object())
        db["titles"] = json::object();
    return db;
}

void save_titles(const json &db) {
    std::ofstream f("v3kn/titles.json");
    f << db.dump(2);
}

void reload_titles_cache() {
    std::lock_guard<std::mutex> lock(titles_cache_mutex);
    titles_cache = load_titles();
}

bool has_title_info(const std::string &titleid) {
    std::lock_guard<std::mutex> lock(titles_cache_mutex);
    return titles_cache.contains("titles") && titles_cache["titles"].contains(titleid);
}

void update_title_info(const std::string &titleid, const json &names) {
    std::lock_guard<std::mutex> lock(titles_cache_mutex);
    if (!titles_cache.contains("titles") || !titles_cache["titles"].is_object())
        titles_cache["titles"] = json::object();

    titles_cache["titles"][titleid]["names"] = names;
    titles_cache["titles"][titleid]["updated_at"] = std::time(0);
    save_titles(titles_cache);
}

std::string get_title_name(const std::string &titleid, const std::string &language) {
    std::lock_guard<std::mutex> lock(titles_cache_mutex);
    if (!titles_cache.contains("titles") || !titles_cache["titles"].contains(titleid))
        return titleid;

    const auto &entry = titles_cache["titles"][titleid];
...
size_t get_titles_cache_size() {
    std::lock_guard<std::mutex> lock(titles_cache_mutex);
    if (!titles_cache.contains("titles") || !titles_cache["titles"].is_object())
        return 0;
    return titles_cache["titles"].size();
}
