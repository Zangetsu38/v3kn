...
static std::unordered_map<std::string, std::vector<json>> pending_friend_status_events; // account_id -> in-memory only status poll events
static std::mutex online_users_mutex;
static std::mutex pending_friend_status_events_mutex;
static std::condition_variable online_monitor_cv;
static std::atomic<bool> monitor_running{ true };
json load_friends(const std::string &online_id, const std::string &group);
...
static void push_friend_status_event(const std::string &account_id, const std::string &target_account_id) {
    std::lock_guard<std::mutex> lock(pending_friend_status_events_mutex);
    json event;
    event["type"] = "friend";
    event["group"] = "status_online";
    event["account_id"] = target_account_id;
    event["at"] = std::time(0);
    pending_friend_status_events[account_id].push_back(event);
}

static json pop_pending_friend_status_events(const std::string &account_id) {
    std::lock_guard<std::mutex> lock(pending_friend_status_events_mutex);
    json result = json::array();
    auto it = pending_friend_status_events.find(account_id);
    if (it != pending_friend_status_events.end()) {
        result = it->second;
        pending_friend_status_events.erase(it);
    }
    return result;
}
...
void notify_friend_poll_for_account(const std::string &account_id) {
    notify_friend_poll(account_id);
}

static void push_status_event_to_friends(const std::string &online_id) {
    ...
    for (const auto &f : user_friends) {
        ...
        push_friend_status_event(friend_account_id, target_account_id);
...
static void monitor_online_users() {
    ...
    while (monitor_running) {
        ...
        // Cleanup old poll events (older than 7 days)
        cleanup_old_poll_events(status_cleanup_age);
    }
}

void register_friends_endpoints(httplib::Server &server) {
    load_poll_events_from_disk();
        ...
        [&](json &events) {
            events.erase(
                std::remove_if(events.begin(), events.end(),
                    [&type, &target_account_id](const json &event) {
                        const std::string event_type = event.value("type", "");
                        const std::string event_group = event.value("group", "");
                        return (event_type == type || (event_type == "friend" && event_group == type))
                            && event.value("account_id", "") == target_account_id;
                    }),
                events.end());
        });
}

// Helper: Pop pending poll events for a user
static json get_friend_events_since(const std::string &account_id, int64_t) {
    return pop_poll_events(account_id);
}
...
void handle_friend_add(const httplib::Request &req, httplib::Response &res) {
    ...
    push_friend_event(target_account_id, "request_received", account_id);
...
void handle_friend_cancel(const httplib::Request &req, httplib::Response &res) {
    ...
    remove_friend_event(target_account_id, "request_received", account_id);
...
void handle_friend_poll(const httplib::Request &req, httplib::Response &res) {
    ...
    while (true) {
        ...
        json pending_status_events = pop_pending_friend_status_events(account_id);
        for (const auto &event : pending_status_events) {
            const std::string target_account_id = event.value("account_id", "");
            if (target_account_id.empty())
                continue;
            ...
            json status_obj = event;
            status_obj.erase("type");
            status_obj.erase("account_id");
            status_obj["online_id"] = target_online_id;
            changes["friend"].push_back(status_obj);
            has_changes = true;
        }
        ...
        if (!events.empty()) {
            ...
            for (const auto &e : events) {
                const std::string event_type = e.value("type", "");
                const std::string group = e.value("group", "");
                std::string target_account_id = e.value("account_id", "");
                if (event_type == "activity") {
                    json notification = e;
                    notification.erase("type");

                    if (group == "commented_on_friend_activity") {
                        const std::string activity_owner_account_id = e.value("account_id", "");
                        if (activity_owner_account_id.empty())
                            continue;

                        const std::string activity_owner_online_id = get_online_id_from_account_id(activity_owner_account_id);
                        if (activity_owner_online_id.empty())
                            continue;

                        notification.erase("account_id");
                        notification["online_id"] = activity_owner_online_id;
                    }

                    changes["activity"].push_back(notification);
                    has_changes = true;
                    continue;
                }
                ...
                if (group == "request_received") {
                    ...
                    if (replace_account_id_with_online_id(event_obj)) {
                        event_obj.erase("type");
                        changes["friend"].push_back(event_obj);
                    }
                    has_changes = true;
                }
            }
        ...
        if (has_changes) {
            ...
            if (changes.contains("friend"))
                update_types.push_back(std::to_string(changes["friend"].size()) + " friend event(s)");
            if (changes.contains("activity"))
                update_types.push_back(std::to_string(changes["activity"].size()) + " activity event(s)");
...
void handle_friend_presence(const httplib::Request &req, httplib::Response &res) {
    ...
    if (status_changed) {
        ...
        if (should_push_online)
            push_status_event_to_friends(online_id);
