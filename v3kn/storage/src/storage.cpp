...
void register_storage_endpoints(httplib::Server &server) {
    ...
    server.Get("/v3kn/check_title_info", handle_check_title_info);
    server.Post("/v3kn/upload_title_info", handle_upload_title_info);
}
...
static void update_trophies_rarity(const std::string &npid, const std::string &npcomm_id) {
    std::lock_guard<std::mutex> lock(trophies_rarity_mutex);

    // Locate the player's trophies.xml
    fs::path trophy_xml_path = fs::path("v3kn") / "Users" / npid / "trophy" / "trophies.xml";

    if (!fs::exists(trophy_xml_path) || fs::is_empty(trophy_xml_path)) {
        log("rarity: trophies.xml missing for NPID " + npid);
        return;
    }
    ...
    if (!result) {
        log("rarity: failed to parse trophies.xml for NPID " + npid + ": " + result.description());
        return;
    }
    ...
    if (!trophies_root) {
        log("rarity: trophies.xml has no <trophies> root for NPID " + npid);
        return;
    }
    ...
    if (std::find(players_array.begin(), players_array.end(), npid) == players_array.end())
        players_array.push_back(npid);

    // Find the <np commid="NPWRxxxxx_00">
    const auto np_node = trophies_root.find_child_by_attribute("np", "commid", npcomm_id.c_str());
    if (!np_node) {
        log("rarity: no <np> node with commid " + npcomm_id + " for NPID " + npid);
        return;
    }
    ...
    if (!unlocked_node) {
        log("rarity: no <unlocked> node for NPID " + npid + " commid " + npcomm_id);
        return;
    }
    ...
    for (auto trophy_node : unlocked_node.children("trophy")) {
        ...
        // Add the NPID to the trophy's array if not already present
        ...
        if (std::find(earned_array.begin(), earned_array.end(), npid) == earned_array.end())
            earned_array.push_back(npid);
    }

    // Save the updated rarity data
    try {
        std::ofstream f(rarity_file);
        f << rarity_json.dump(4);
        log("rarity: updated rarity stats for NPID " + npid);
    } catch (...) {
...
void handle_upload_file(const httplib::Request &req, httplib::Response &res) {
    ...
    if (type == "trophy")
        update_trophies_rarity(npid, id);
...
void handle_check_trophy_conf_data(const httplib::Request &req, httplib::Response &res) {
    ...
    for (pugi::xml_node np_node : doc.child("trophies").children("np")) {
        ...
        const auto mark_missing = [&](const std::string &reason) {
            ...
            log("NPID " + npid + " " + reason + " for: " + commid);
        };
...
void handle_check_title_info(const httplib::Request &req, httplib::Response &res) {
    std::lock_guard<std::mutex> req_lock(request_mutex);

    std::string err;
    const std::string npid = get_valid_npid(req, "title info check", err);
    if (npid.empty()) {
        res.set_content(err, "text/plain");
        return;
    }
    ...
    if (titleid.empty()) {
        log("Missing TitleID on title info check for NPID " + npid);
        res.set_content("ERR:MissingTitleID", "text/plain");
        return;
    }

    if (has_title_info(titleid)) {
        log("Title info exists for TitleID " + titleid + ", NPID " + npid);
        res.set_content("OK:TitleInfoExists", "text/plain");
        return;
    }

    log("Title info missing for TitleID " + titleid + ", NPID " + npid);
    res.set_content("WARN:TitleInfoMissing", "text/plain");
}

void handle_upload_title_info(const httplib::Request &req, httplib::Response &res) {
    std::lock_guard<std::mutex> req_lock(request_mutex);

    std::string err;
    const std::string npid = get_valid_npid(req, "title info upload", err);
    if (npid.empty()) {
        res.set_content(err, "text/plain");
        return;
    }
    ...
    if (req.body.empty()) {
        log("NPID " + npid + " try to upload title info with missing payload");
    ...
    try {
        ...
        log("NPID " + npid + " try to upload title info with invalid JSON");
    ...
    if (titleid.empty() || !payload.contains("names") || !payload["names"].is_object()) {
        log("NPID " + npid + " try to upload title info with invalid payload");
        res.set_content("ERR:InvalidPayload", "text/plain");
        return;
    }

    update_title_info(titleid, payload["names"]);

    log("NPID " + npid + " uploaded title info for " + titleid);
    res.set_content("OK:TitleInfoSaved", "text/plain");
}
