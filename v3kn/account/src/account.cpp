...
static std::string lowercase_npid(std::string npid) {
    std::transform(npid.begin(), npid.end(), npid.begin(), [](unsigned char c) {
        return static_cast<char>(std::tolower(c));
    });
    return npid;
}

static std::string find_existing_npid_case_insensitive(const json &users, const std::string &npid) {
    const std::string needle = lowercase_npid(npid);
    for (const auto &[existing_npid, _] : users.items()) {
        if (lowercase_npid(existing_npid) == needle)
            return existing_npid;
    }
    return {};
}

std::mutex profile_mutex;
void update_profile_timestamp(const std::string &npid) {
    std::lock_guard<std::mutex> lock(profile_mutex);

    const fs::path profile_path{ fs::path("v3kn") / "Users" / npid / "profile.json" };
    ...
    // Save
    {
        ...
        profile_file_out << profile.dump(4);
    }
}
...
void handle_create_account(const httplib::Request &req, httplib::Response &res) {
    ...
    json db = load_users();
    const std::string existing_npid = find_existing_npid_case_insensitive(db["users"], npid);
    if (!existing_npid.empty()) {
...
void handle_login(const httplib::Request &req, httplib::Response &res) {
    ...
    const std::string login_npid = trim_npid(req.get_param_value("npid"));
    if (login_npid.empty()) {
    ...
    const std::string base64_password = req.get_param_value("password");
    if (base64_password.empty()) {
        log("Missing password on login attempt for NPID " + login_npid);
        res.set_content("ERR:MissingPassword", "text/plain");
        return;
    }
    ...
    const std::string npid = find_existing_npid_case_insensitive(db["users"], login_npid);
    if (npid.empty()) {
        log("Login attempt for non-existing NPID " + login_npid);
    ...
    res.set_content("OK:" + npid + ":" + token, "text/plain");
}
...
void handle_change_npid(const httplib::Request &req, httplib::Response &res) {
    ...
    std::lock_guard<std::mutex> lock_db(account_mutex);
    json db = load_users();
    const std::string existing_npid = find_existing_npid_case_insensitive(db["users"], new_npid);
    if (!existing_npid.empty() && (existing_npid != npid)) {
...
void handle_upload_panel(const httplib::Request &req, httplib::Response &res) {
    ...
    std::string err;
    const std::string npid = get_valid_npid(req, "panel upload", err);
    if (npid.empty()) {
        res.set_content(err, "text/plain");
        return;
    }
    ...
    if (!req.form.has_file("file")) {
        log("Missing file on panel upload for NPID " + npid);
        res.set_content("ERR:MissingFile", "text/plain");
        return;
    }
    ...
    const auto file = req.form.get_file("file");
    if (file.content.empty()) {
        log("Empty file on panel upload for NPID " + npid);
        res.set_content("ERR:EmptyFile", "text/plain");
        return;
    }
    ...
    // Max 2MB
    if (file.content.size() > 2 * 1024 * 1024) {
        log("Panel too large for NPID " + npid + " (" + std::to_string(file.content.size()) + " bytes)");
        res.set_content("ERR:FileTooLarge", "text/plain");
        return;
    }

    // Verify PNG signature and dimensions (360x72)
    const auto &data = file.content;
    const uint8_t png_sig[] = { 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A };
    if (data.size() < 24 || std::memcmp(data.data(), png_sig, 8) != 0) {
        log("Invalid PNG file on panel upload for NPID " + npid);
        res.set_content("ERR:InvalidPNG", "text/plain");
        return;
    }
    ...
    if (width != 360 || height != 72) {
        log("Panel dimensions invalid for NPID " + npid + " (" + std::to_string(width) + "x" + std::to_string(height) + ")");
        res.set_content("ERR:DimensionsTooLarge", "text/plain");
        return;
    }

    const fs::path panel_path = fs::path("v3kn") / "Users" / npid / "Panel.png";
    ...
    update_last_activity(req, npid);
    log("Panel uploaded for NPID " + npid + " (" + std::to_string(file.content.size()) + " bytes)");
...
void handle_get_panel(const httplib::Request &req, httplib::Response &res) {
    ...
    std::string err;
    const std::string npid = get_valid_npid(req, "panel download", err);
    if (npid.empty()) {
    ...
    const fs::path panel_path = fs::path("v3kn") / "Users" / lookup_npid / "Panel.png";
    if (!fs::exists(panel_path)) {
        log("Panel not found for NPID " + lookup_npid + " requested by NPID " + npid);
    ...
    log("Panel downloaded for NPID " + lookup_npid + " requested by NPID " + npid);
